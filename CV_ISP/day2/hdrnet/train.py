...
def train(params=None):
    ...
    train_dataset = HDRDataset(params['dataset'], params=params, suffix=params['dataset_suffix'], mode='train')
    val_dataset = HDRDataset(params['dataset'], params=params, suffix=params['dataset_suffix'], mode='val')
    ...
    for e in range(params['epochs']):
        e = e + prev_e
        model.train()
        for i, (low, full, target, _, _) in enumerate(train_loader):
            optimizer.zero_grad()
        ...
        for i, (low_val, full_val, target_val, _, _) in enumerate(val_loader):
            ...
            val_ssim += float(ssim(res_val, t_val, data_range=1, size_average=True))
            val_count += 1
